...
\[  \vec{D} =  \frac{\epsilon \vec{E}}{\mu \vec{H}}  = \vec{B}\]
...
\[ \vec{k} \cdot \vec{r} - \omega{t} = cte  \]



\[ \vec{k} \cdot \vec{r} = cte (fixed)  \]


\textbf{Acá va una imagen}
...
\[ \hat{n} \cdot \vec{x} = \vec{n}\cdot \vec{p} =cte  \]
...
\[ \cos(\vec{k}\cdot \vec{t} - \omega{t})   = \frac{e^{i (\vec{k\cdot \vec{r}  } -\omega{t})} + e^{- i(\vec{k}\cdot \vec{r} - \omega{t})}}{2}  \]
...
\[    \vec{E} = \vec{A} \left[  \cos(  \vec{k} \cdot \vec{r} -\omega{t}) + i\sen(\vec{k}\cdot \vec{r}-\omega{t})   \right] \]


If \[  \vec{K} = \vec{K}_{r} + i \vec{k}_{i}, \quad \text{then}  \]

\[  \vec{E} = \vec{A}e^{i(\vec{k}_{r} \cdot \vec{r})} e^{-\vec{k}_{i}\cdot \vec{r}} e^{i\omega{t}}\]
...
Hacer en algun lenguaje de programación la gráfica:


\textbf{Objetivo 1:}

Plot the electric field for $(\vec{E}(\vec{r}))$.

\textbf{Objetive 2:} 

Plot the EM energy density.

Tanto la $A$, $K$ y $\omega$ son reales para este proyecto final. O son vectores reales. 

\subsection{Proyecto final es en equipos}. 

Vamos a hacer los cálculos en equipos:



    \textbf{Activity 1}
    \begin{itemize}
        \item[$\bullet$] Obtain the wave equation for dielectric media.
    \end{itemize}

     \textbf{Activity 2}
    \begin{itemize}
        \item[$\bullet$] Show that a plane wave is a solution to the wave equation.
        \item[$\bullet$] Show that $\vec{k}$, $\vec{E}$ and $\vec{H}$ are perpendicular.
    \end{itemize}

    \textbf{Activity 3}
    \begin{itemize}
        \item[$\bullet$] Assuming that $\vec{E} = \vec{A}\, e^{i(\vec{k}\cdot\vec{r} - \omega t)}$, obtain the relation for $E_{x}$ and $H_{y}$.
        \item[$\bullet$] Assume that $\vec{E} = (E_{x}, 0, E_{z})$ and $\vec{H} = (0, H_{y}, 0)$.
\end{itemize}
