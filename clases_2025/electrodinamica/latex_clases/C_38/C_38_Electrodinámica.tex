...
\section{Regla de la mano derecha}


\subsection{Torcas de tarea}


\[ \vec{\tau} = (\tau_{x},\tau_{y},\tau_{z})  \]


Tenemos que definir un vector área. 


EL circuito cerrado, no es plano en general, si lo proyectamos sobre el plano $x-y$. Vemos un cierto plano sobre la proyección, podemos proyectarlo. 



Nosotros teníamos un elemento de fuerza:
...
\[   d\vec{\tau} = I\vec{r} \times (d\vec{\ell}\times \vec{B})    \]




\[ \tau = I \oint_{C} \vec{r} \times (d\vec{\ell}\times \vec{B} )  \]


\[ \oint_{C} \eta d\eta =0 \]


\[ \oint_{C} \epsilon d\eta =A \]


Tenemos:


\subsection{Vector área unsando la quilaridad derecha}


\subsubsection{Componente x}


\[ \oint_{C} \vec{r} \times (d\vec{\ell}  \times \vec{B} )_{x} =ydxB_{y}-\cancelto{0}{ydyB_{z}}-\cancelto{0}{zdzB_{x}}+zdxB_{z}  \]


¿Cuál da un término negativo debido a la quilaridad derecha?

Tenemos:

\[  = AyB_{z}-A_{z}B_{y} \quad \text{componente x}  \]


Algo similar se puede hacer para el resto de las componentes. 

En general decimos que la $\tau$ se calcula como:
...
Pensamos como físicos teóricos, alguna cantidad que nos permita caracterizar el efecto sobre el campo magnético.


Definimos una cantidad:


\[  \boxed{ \vec{m} = I\vec{A}  } \quad \text{Magnetic moment of a circuit}  \]

Idea generalizadora, que nos permiten caracterizar a sistemas. En este caso es un circuito con corriente, ahora vamos a hacer una integral y vamos a obtener el caso más general de un momento magnético en términos de una integral cerrada sobre un circuito.
...
\[ \oint_{C} \vec{r} \times d\vec{\ell} \propto \vec{A} \]
...
\[  \oint_{C} (r_{x},r_{y},0) \times (dl_{x},dl_{y},0)  \propto (x,y,0)\]
...
\[ \frac{1}{2} \oint_{C} \vec{r} \times d\vec{\ell} = \vec{A} \]
...
\[    \vec{m} = \frac{I}{2} \oint_{C} \vec{r} \times d\vec{\ell}   \]

\[ d\vec{m} = \frac{\vec{r}}{2} \times Id\vec{\ell}  \]

\textbf{Revisar la relación entre corriente eléctrica}

Tenemos:


\[\boxed{ d\vec{m} = \frac{\vec{r}}{2} \times \vec{J}dv}   \]
