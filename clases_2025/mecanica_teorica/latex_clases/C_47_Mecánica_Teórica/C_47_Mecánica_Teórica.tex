...
\title{Clase \# 47 - Mecánica Teórica}
...
\[ S(x,t,P) = \pm \int\limits \sqrt{ P^{2}-m^{2}\omega^{2}x^{2}  } dx - \frac{P^{2}}{2m}t  \]
...
\underline{Transformación canónica}
...
\[ Q = \frac{\partial S}{\partial P} = \pm \int\limits \frac{1}{2} \left( \frac{2P dx  }{  \sqrt{ P^{2}-m^{2}\omega^{2}x^{2}  }  } - \frac{P}{mt} \right) t\]
...
\[ p = \pm \sqrt{ P^{2} - m^{2}\omega^{2}x^{2}} \]


Tenemos:


\[ Q = \pm \left(   \frac{ P  }{ m\omega } \right)\arcsin \left(  \frac{m\omega x}{P}  \right) - \frac{P}{m} t \]

Tenemos:


\[ P = \sqrt{  p^{2} + m^{2}\omega^{2}x^{2}  }  \]


Tenemos $\ldots$

\[Q = \left(  \frac{ \sqrt{ p^{2}+m^{2}\omega^{2}x^{2}  }   }{ m\omega  }   \right)\arcsin \left( \frac{ m\omega x  }{\sqrt{ p^{2}+m^{2}x^{2}  }}  \right)  - \left( \frac{ \sqrt{ p^{2} + m^{2} \omega^{2} x^{2}  }  }{m\omega}   \right)t \]


Tenemos:

\[ x(t) = \left(   \frac{ P  }{\omega{w}} \right) \arcsin   \]


Partícula libre con oscilador armónico no solo a nivel clásico, sino también a nivel cuántico. 



\subsection{Partícula libre en Hamilton-Jacobi}
...
\[ H(x,\frac{\partial S}{\partial  x}, t  ) + \frac{\partial S}{\partial S} = 0 \]
...
Borramos los subíndices por que pueden ser cualquier cosa para la solución, entonces tenemos:
...
Sí ya es \textbf{integral completa} de la \textbf{ecuación de hamilton Jacobi} para partícula libre.


\[ \boxed{S(x,t,x_{0}) = \frac{1}{2} m \frac{ (x-x_{0})^{2}  }{t}  }  \]

Tenemos:


Por lo tanto que la $S$ es una función generatriz que la manda a una:


POdemos tomar como constate a la corrdenada:



\[ P = x_{0}\]

Aparece en la ecuación de Hamilton Jacobi.

\begin{center}
	\includegraphics[width=0.8\linewidth]{4.png} % <-- Sustituye esta ruta
\\[2mm]
\textbf{.}
\end{center}	
...
\[  p = \frac{\partial S}{\partial x} = \frac{m}{t} (x-P) \]


\[  Q = \frac{\partial S}{\partial P} = - \frac{m}{t} (x-P) \]
...
\[ (x,p) \quad \leftrightarrow \quad (Q,P) \]
...
\emph{Las transformaciones canónicas son interacciones} tienen inmersa una interacción. Es una forma de colocar las interacciones. 

Por lo tanto, por ejemplo en el caso de un objeto en caida libre.

Depende mucho del observador. 

\textbf{Lentes gravitacionales}.


Teoremas generales. 

Múltiples imágenes de un solo objeto. 


Pratones de Roche. Aberración esférica, simetrías de revolución. 


Mañana va a partir de la ecuaión de Shrodinger. 


\textbf{Ecuación de continuidad}.

Constricciones no holonomas.

Cuerpo rigido.

ángulo acción.

Geometría diferencial, teoría de grupos. 
