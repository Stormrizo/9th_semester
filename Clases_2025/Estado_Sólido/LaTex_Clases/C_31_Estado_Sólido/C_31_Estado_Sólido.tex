...
\section{Un punto del espacio recíproco ocupa esta longitud $\Delta{k} = \frac{\pi}{L}$}
...
\[  \omega = 2 \sqrt{  \frac{ \beta}{m}  \left| \sen \frac{ka}{2} \right|}  \]

Debye considera como aproximación que la velocidad del sonido es constante

Twcuerde que el límite de la londitd de onda larga:

\[ \omega = \nu k \]

Además para $N$ osciladores:

\[ N= \int\limits_{0}^{\omega{D}} g(\omega) d\omega \]
...
\textbf{Tarea: verificar las unidades - ánalisis dimensional}


\[ E = \int\limits_{0}^{\omega{D}} \frac{  \hbar\omega g(\omega) }{ e^{ \hbar \omega / K_{B}T } - 1 } d\omega = \frac{L}{\pi v} \int\limits_{0}^{\omega D}  \frac{  \hbar \omega }{ e^{\hbar \omega / K_{B}T} -1} d\omega \]

Energía de una cadena monoatómica usando el modelo de Debye.

Se obtien $C_{v}$ derivando la expresión anterior respecto de $T$
...
\subsection{¿Qué ocurre en el caso de un cristal (3D)?}
...
\textbf{Condiciones de frontera anteriores}. Traer esta última parte de las notas subidas en teams. 

Tarea de conceptos

Energía interna

Capacidad calorifiac
