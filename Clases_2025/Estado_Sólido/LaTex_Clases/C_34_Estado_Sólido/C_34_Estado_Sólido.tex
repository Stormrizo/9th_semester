...
\section{Densidades de carga} \vspace{0.5cm}


Si $E_{ext} \neq 0$ y constante, la $\langle v \rangle \neq 0$ y en dirección opuesta al campo.

La fuerza sobre el electrón debida al campo externo es:

\[ F_{e} = -eE_{ext} = m \frac{d}{dt} \langle v \rangle \approx m \frac{\Delta{v}}{\Delta{t}}   \]



$\Delta{t}$

Mantener esa idea nos ayuda a expresar a 

\[ \Delta{t} \approx \tau, \Delta{v}  \] es el cambio de velocidad al colisionar, así que de manera aproximada tenemos:

\[\langle \mathbf{V} \rangle = - \frac{ eE_{ext}  }{m}\tau \], por lo que la densidad de corriente es:


\[  J = -\eta e \langle \mathbf{v} \rangle = -\eta e \left(   - \frac{eE_{ext}}{m} \eta  \right)  = \frac{ \eta e^{2}\tau }{m} E_{ext}  \]




Tiempo de relajación, tiempo o intervalo entre una colisión u otra. 

Sabemos que $J=\sigma E_{ext}$ comparando (A) y (B) tenemos:


\[ \sigma_{DC} = \frac{ \eta e^{e}\tau }{m}  \]

¿Cuánto debe de valer la $\sigma - \text{conductividad}$? ¿Cuánto debe de valer la $\rho - \text{resistividad}$?

¿Cómo se mide la resistencia?

Para el día lunes buscar las unidades de la conductividad, resisitvidad, buscar mínimo dos ejemplos. 


Experimentalmente se mide la resistividad:

\[ \rho = \frac{1}{\sigma}  \]

Y con ello se logra tener una estimación del tiempo de relajación

\[ \boxed{ \tau = \frac{ m }{\rho \eta e^{2}  }  } \]


El cual para metales suele ser del ordem de $10^{-14}s$

$\boxed{\text{ Ecuación de movimiento de los electrones}}$

Surgen modelos interesante considerando:


\[ \tau(T) \quad \to \quad \sigma(T)  \]

A partir del modelo de Drude es establece que la ecuación de movimiento de los elecroes en presencia de la red de iones positivos es:


\[ \boxed{ \frac{ d \mathbf{p(t)} }{dt} = - \frac{ \mathbf{p}(t) }{\tau} + F_{ext}(t)    } \]

Fueza externa debida al campo externo. 



Tenemos: \textbf{Fuerza de la red}

\[ \vec{F}_{red} = - \frac{ \vec{P}(t) }{\tau} = - \frac{m\vec{v}}{\tau} 


¿Qué fuerzas son proporcionales a la velocidad? 
