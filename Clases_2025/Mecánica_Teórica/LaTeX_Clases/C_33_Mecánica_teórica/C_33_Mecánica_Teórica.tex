...
\title{Clase \# 33 - Mecánica Teórica}
...
\[  f(x), \quad g(p) \equiv px(p) - f(x(p)) \]

\[ F(x,p) \equiv xp-f(x)  \]

\[ \frac{ \partial F }{\partial x} = 0 \quad \Rightarrow \quad p - \frac{df}{dx}=0 \]

\[  \Rightarrow p = \boxed{\frac{df}{dx}  }  \]

\[ \Rightarrow \quad x=x(p)   \]
...
En partícula libre potencial estamos sonsiderando igual a 0, entonces la energía cinpetica es igual a la energía total.


\[ H(p) \equiv \frac{p^{2}}{2m}  \]


Antes de eso tenemos mediante transformada de Legendre:


\[ F(\dot{x}, p) = \dot{x}p - \frac{1}{2}m\dot{x}^{2}  \]

\[ \frac{ \partial F }{\partial \dot{x}} = 0 \quad \Rightarrow \quad p-m\dot{x} =0  \]


\[\boxed{ p=m\dot{x} } \quad \Rightarrow \quad \dot{x} = \dot{x}(p) = \boxed{ \frac{p}{m} }  \]

Tenemos:


\[ H(p) \equiv  \frac{ p^{2} }{m} - \frac{1}{2} m \frac{ p^{2} }{m^{2}} = \frac{2p^{2}}{2m} - \frac{p^{2}}{2m} = \boxed{\frac{p^{2}}{2m}  }\]


Tenemos:

\[L = \overbrace{\frac{1}{2} m \dot{x}^{2} - \frac{1}{2} kx^{2}}^{T-V}  \]


\[ F(\dot{x}, p) \equiv \dot{x}p - \frac{1}{2}m\dot{x}^{2} + \frac{1}{2}kx^{2} \]


\[ \frac{ \partial F }{\partial \dot{x}} = p-m\dot{x} =0 \]

\[  \boxed{ \dot{x} = \frac{p}{m} } \]

\[ H(x,p) = \frac{p^{2} }{m} - \frac{1}{2}m \frac{p^{2}}{m^{2}} + \frac{1}{2} kx^{2} \]
...
¿Cómo se le ha ocurrido a Newton el hecho de que los planetas sientas atracción?

Tenemos:


\[ \boxed{H = \frac{p^{2}}{2m} + \frac{1}{2}kx^{2}} = E \]

\[ \frac{ p^{2} }{2mE} + \frac{kx^{2}}{2E}=1 \]

Ec. se una elipse en el espacio fase.

\[  \frac{x^{2}}{a^{2}} + \frac{y^{2}}{b^{2}} = 1 \quad \to \quad \frac{1}{2mE} = \frac{1}{b^{2}} \quad \frac{k}{2E} = \frac{1}{a^{2}}  \]


Que $x$ y $p$ que sean iguales, significa que no podemos distinguirlos, son etiquetas para etiquetar un punto en el plano.

Esto es lo que sucede con $x$ y $p$ en formulación hamiltoniana. 


Supongamos que tenemos la siguiente lagrangiana:

\[ L = L(q,\dot{q},t)   \]

En donde el lagrangiano es cuadratico en la coordenada generalizada

Entonctramos el hamiltoniano que depende de todas las variables que depende el lagrangiano.

\subsection{Transformada de Legendre}


Construimos una función auxiliar:


\[  F(\dot{q},p) \equiv \dot{q}p-L    \]


\[ \frac{ \partial F}{ \partial \dot{q} } =0 \quad \Rightarrow \quad \underbrace{\boxed{ p = \frac{\partial L  }{\partial \dot{q}  }}}_{ \dot{q} = \dot{q}(p,q,t)  }  \]


Entonces el hamiltoniano es:


\[ H(q,p,t) \equiv \dot{q} (q,p,t) p - L(q,\dot{q}(q,p,t),t)   \]


Si tenemos:


la lagrangiana siguiente:


\[ L = L(q_{i},\dot{q}_{i}, t)  \]

\[ H(q_{i}, p_{i}, t) \equiv \sum\limits_{j=1}^{n} p_{i}\dot{q}_{i}(q_{j}, p_{j}, t) - L(q_{i},\dot{q}_{i} (q_{j}, p_{j},t) )\]

Lagrangiano no singular

TRansformaciones de punto en el espacio de configuraciones. 
